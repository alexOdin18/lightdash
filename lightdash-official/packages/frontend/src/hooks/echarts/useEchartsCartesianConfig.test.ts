...
describe('getCategoryDateAxisConfig', () => {
    ...
    describe('YEAR interval', () => {
        test('uses unique sorted raw values from rows (matches dataset for chart rendering)', () => {
            const result = getCategoryDateAxisConfig(
                axisId,
                createAxisField(TimeFrames.YEAR),
            ...
            expect(result.data?.[1]).toContain('2026');
        });
    ...
    describe('QUARTER interval', () => {
        test('uses unique sorted raw values from rows', () => {
            const result = getCategoryDateAxisConfig(
                axisId,
            ...
            expect(result.data?.[1]).toContain('2024-10');
        });
        ...
        test('handles quarters spanning year boundary', () => {
            ...
            expect(result.data).toHaveLength(2);
            expect(result.data?.[0]).toContain('2024-10');
            expect(result.data?.[1]).toContain('2025-04');
        });
    });
    ...
    describe('MONTH interval', () => {
        test('uses unique sorted raw values from rows', () => {
            ...
            expect(result.data).toHaveLength(2);
            expect(result.data?.[0]).toContain('2024-01');
            expect(result.data?.[1]).toContain('2024-03');
        });
        ...
        test('handles months spanning year boundary', () => {
            ...
            expect(result.data).toHaveLength(2);
            expect(result.data?.[0]).toContain('2024-11');
            expect(result.data?.[1]).toContain('2025-02');
        });
    });
    ...
    describe('WEEK interval', () => {
        test('uses unique sorted raw values from rows', () => {
            ...
            expect(result.data).toHaveLength(2);
            expect(result.data?.[0]).toContain('2024-01-01');
            expect(result.data?.[1]).toContain('2024-01-22');
        });
    });
});
