...
/**
 * Collect unique raw values for an axis from rows and sort them chronologically.
 * Uses actual data values so category axis matches the dataset (ECharts requires exact match).
 */
const getUniqueSortedRawDateValues = (
    axisId: string,
    rows: ResultRow[],
    pivotDetails?: InfiniteQueryResults['pivotDetails'],
): (string | number)[] => {
    const columnNames = getColumnNamesForField(axisId, pivotDetails);
    const byKey = new Map<string, string | number>();
    for (const columnName of columnNames) {
        for (const row of rows) {
            const raw = row[columnName]?.value?.raw;
    ...
    return unique.sort((a, b) => {
        const tA = dayjs.utc(a).valueOf();
        const tB = dayjs.utc(b).valueOf();
...
/**
 * Generate category axis config for date intervals (WEEK, MONTH, QUARTER, YEAR).
 * Uses actual unique raw values from the dataset so ECharts can match series to the axis.
 * Previously used a generated continuous range (dayjs.format()) which did not match
 * value.raw from the backend, so the chart did not render.
 */
export const getCategoryDateAxisConfig = (
    ...
    if (
        ...
        const categoryData = getUniqueSortedRawDateValues(
            axisId,
            rows,
            pivotDetails,
        );
...
const useEchartsCartesianConfig = (
    ...
    // ECharts category axis matches by strict equality. Dataset must use primitive values
    // (value.raw) so that x/y encode values match the axis data (e.g. MONTH/YEAR).
