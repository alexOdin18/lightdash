...
This project includes a **Lightdash semantic layer**: metrics, dimensions, and model relationships are defined in `models/schema.yml` (under `config.meta`) and in `lightdash.config.yml`. After `dbt compile` or `dbt run`, Lightdash can read the manifest and expose the same metrics for self-service analytics.

**Included metrics (examples):**
- **Orders:** Total orders, Completed orders, Total revenue, Completed revenue, Average order value, Fulfillment rate
- **Customers:** Unique customers, Total customer value, Average customer value

**Note:** Lightdash does not support DuckDB as a warehouse. To run queries from Lightdash you need a supported warehouse (e.g. Postgres). You can use the same dbt project structure and semantic definitions with a Postgres profile and load the same data there, or use the [full-jaffle-shop-demo](https://github.com/lightdash/lightdash/tree/main/examples/full-jaffle-shop-demo) which uses Postgres. See `LIGHTDASH.md` in this repo for setup options.
