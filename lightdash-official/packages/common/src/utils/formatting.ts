...
/**
 * Calendar intervals: we show year/month/quarter/day in the value's timezone.
 * Converting to UTC would shift the calendar date (e.g. 2018-01-01 00:00 +0300
 * becomes 2017-12-31 21:00 UTC and would display as 2017).
 */
const CALENDAR_DATE_INTERVALS: TimeFrames[] = [
    TimeFrames.YEAR,
    TimeFrames.MONTH,
    TimeFrames.QUARTER,
    TimeFrames.DAY,
...
export function formatDate(
    ...
    const isCalendarPart =
        timeInterval &&
        CALENDAR_DATE_INTERVALS.includes(timeInterval);

    let momentDate: moment.Moment;
    if (displayTimezone && isCalendarPart && dayjs.tz) {
        // Value from DB driver is typically UTC; interpret it in project timezone so calendar date is correct.
        const d = dayjs.utc(date);
        if (!d.isValid()) return 'NaT';
        const inTz = d.tz(displayTimezone);
        return inTz.isValid() ? inTz.format(getDateFormat(timeInterval)) : 'NaT';
    }

    const useUTC = convertToUTC && !isCalendarPart;
    momentDate = useUTC ? moment(date).utc() : moment(date);
